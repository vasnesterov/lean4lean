...
| `SetModel/InstDescendAudit.lean` | the recorded `sort_inst` witness machine-checked and shown **symmetric** (`w_head_type_agree`, `w_types_of_sortPiConv`); `SortInstDescend`/`SortInstDescend0` and `sortInstDescend_iff` — the level condition is free given `Ordered` + `PropUniq`; `sortInstDescend0_bvar_forces_sort` — the `.bvar k` case IS uniqueness of typing |

...
| 75c | **row 68's own `member`-not-slack bound** | **it does not cover the case that matters, and the stream corrected itself.** `semResidue_not_member_badTy` shows `member` is not slack in `RestoreData ∧ OccData ∧ hl ∧ ha` — but **not** in those *plus* `occurs`, because the witness violates `occurs` too (`pfnOccBadTy_not_occurs`, machine-checked: `Occurs.ty_const` pins exactly the perturbed type). Whether `member` is slack given `occurs` turns on whether `VInductDecl'.Declared` pins `(occ j).src.indices`, and `Theory/Inductive/Decl.lean:694`'s own docstring lists `VIndType.indices` among the fields **unrecoverable** from a declaration — pinned only up to the recursor type's `IsDefEqU`. No witness built, and none looked for. This is row 11a's lesson recurring: **a non-slackness bound is only as strong as the hypothesis set it holds the field against** | `pfnOccBadTy_not_occurs` (`NestedOccData.lean`) | n/a |
...
| 76a | `hargs`'s factorability | **factorable, to exactly two data, and two is forced by the construction rather than the statement.** `hbody_weak` (which is `IsDefEq.weakR` *applied* — named only to record the reduction, not as new content) weakens one typing at the params-only context to every entry's context. So `hargs` enters once per `Faithful` clause — `ty_agree`'s spine for the motive block, `ctor_agree`'s for the minor block — not once per block and not once per entry. It cannot be one, because the two blocks apply **different constants with different declared types**, so no single `HasArgs` serves both | `VIndRestore.hbody_weak` (same file) | n/a |

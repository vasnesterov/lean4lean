...
a survey then found four more already refuted elsewhere in the tree. Forty-one statements are
...
| 11b | `ModelFitsInput` / `CnstRecursion.upper_bound_of` | **row-2-shaped**: a proved theorem whose second hypothesis is false exactly when it has anything to say | `not_modelFitsInput (hA : InaccModelInput) (hc : Consistent 𝗭𝗙𝗖+𝗜𝗻𝗮𝗰𝗰) : ¬ ModelFitsInput` — and the contradiction does *not* route through the proof-split input | n/a |

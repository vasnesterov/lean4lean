...
import Lean4Lean.Theory.SetModel.EqTypeFormer  -- 2026-09-02: the `.induct eqIndDecl` step's TYPE-FORMER cell, proved -- the last of `InductOracleOK`'s three `consts` cells at this block. Imported so it is not orphaned: `EqTFAudit.mem_interp_EqType_of_eqFn` discharges the `consts` obligation at `Eq` itself, and `EqTFAudit.eqSpec_not_sufficient` is the control showing this cell (unlike the other two) is NOT derivable from `EqSpec` -- it needs the value's identity (`preludeWitness_cnst_eq`), not its applied values.
import Lean4Lean.Theory.SetModel.EqRecNecessity  -- 2026-09-02: `EqSpec` is NECESSARY at `eqIndDecl`'s RECURSOR cell -- the expensive control handoff §17.8 item 2 left unbuilt and `PreludeOracle.lean:905` asserts only in prose. Imported so it is not orphaned: `EqRecNec.not_pt_mem_interp_eqRecType_badTrue` refutes `EqZeroAudit.pt_mem_interp_eqRecType_of_zero`'s conclusion at a constant-true `Eq`, so that theorem's hypothesis is bracketed on both sides.
import Lean4Lean.Theory.SetModel.EqRecLarge  -- 2026-09-02: the `.induct eqIndDecl` step's `≠ 0` SLICE -- the six-layer `mkLam` at `Eq.rec` that `EqAudit.pt_not_mem_interp_eqRecType_of_ne` shows `•` cannot serve. Imported so it is not orphaned.
import Lean4Lean.Theory.SetModel.PreludeRecGap  -- 2026-09-02: the shared witness at the three RECURSOR cells, measured. Imported so it is not orphaned: `RecGap.preludeWitness_not_mem_interp_iffRecType` extends EqRecLarge §8's refutation from `Eq.rec` to `Iff.rec`, `RecGap.preludeWitness_mem_interp_neRecType` shows `Nonempty.rec` is NOT affected (so the repair is two cells, not three), and `RecGap.eqRec_arm_ne_empty` is the control that keeps `SetModel.preludeWitness_cnst_empty`'s two recursor hypotheses from being read as defensive -- the shared witness' `Eq.rec` entry really is not `∅` on the `≠ 0` slice. (§§3--4's `preludeWitnessR` measuring instrument was retired 2026-09-03: the repair it priced is installed at `SetModel.preludeWitness`.)
import Lean4Lean.Theory.SetModel.IffRecLarge  -- 2026-09-02: the `.induct iffIndDecl` step's `≠ 0` SLICE -- the five-layer `mkLam` at `Iff.rec`. Imported so it is not orphaned: `IffLargeAudit.iffRecFn_mem_interp_iffRecType` settles `PreludeRecGap` §6's open question affirmatively (the cell is SATISFIABLE, so `RecGap.preludeWitness_not_mem_interp_iffRecType` is not free), and `IffLargeAudit.iffRecCell_discriminates` is its anti-vacuity control.
import Lean4Lean.Theory.SetModel.IffConsts  -- 2026-09-03: the `.induct iffIndDecl` step's other two `consts` cells -- the TYPE FORMER and `Iff.intro`. Imported so it is not orphaned: `IffConstsAudit.inductOracleOK_consts_Iff` completes `InductOracleOK`'s `consts` field at that block at the shared `preludeWitness`, and `IffConstsAudit.eq_of_mem_mpTy_mprTy` is the step with content -- `Iff.intro`'s cell is true only because its two field binders force `a = b`, so the cell is not derivable from the result type alone.
import Lean4Lean.Theory.SetModel.EqIotaRule  -- 2026-09-03: the `.induct eqIndDecl` step's **ι-RULE** -- `InductOracleOK`'s `rules` field at this block, the field no prelude block but `nonemptyIndDecl` had. Imported so it is not orphaned: `EqIotaAudit.defEqOK_eqRule` discharges `DefEqOK` at `eqIndDecl`'s single ι-rule at the shared `preludeWitness`, and `EqIotaAudit.eqRule_discriminates` is the control showing the `≠ 0` slice's membership half is not satisfied by `•`, the value the `= 0` slice hands the same cell.
import Lean4Lean.Theory.SetModel.IffIotaRule  -- 2026-09-03: the `.induct iffIndDecl` step's **ι-RULE** -- `InductOracleOK`'s `rules` field at the second prelude block, and with it the whole structure at that block. Imported so it is not orphaned: `IffIotaAudit.inductOracleOK_Iff` assembles `InductOracleOK` at `iffIndDecl` at the shared `preludeWitness`, `IffIotaAudit.iffRule_discriminates` is the control showing the `≠ 0` slice's membership half is not satisfied by `•` (the value the `= 0` slice hands the same cell), and `IffIotaAudit.mp_eq_pt_of_mem` is the step with content -- both constructor fields are forced to `•` because their types are propositions, which is what makes the lhs' `min ‘ • ‘ •` and the rhs' `min ‘ mp ‘ mpr` the same element.

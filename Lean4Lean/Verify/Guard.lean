...
1. **Axiom freeze**: `Lean4Lean/Verify/Axioms.lean` declares exactly the 32
   axioms it declared at commit `20e2d14`.
2. **Axiom whitelist**: the axioms of `kernel_sound` are contained in
   {`propext`, `Classical.choice`, `Quot.sound`} ∪ the 32 frozen axioms
...
/-- The 32 axioms declared by `Lean4Lean/Verify/Axioms.lean` as of commit
`20e2d14` (the frozen whitelist from `CLAUDE.md`), plus the three standard
axioms. -/
def axiomWhitelist : List Name := [
  ``propext, ``Classical.choice, ``Quot.sound,
  -- the 32 frozen axioms
...
/- Check 1: the frozen axiom file declares exactly the 32 whitelisted axioms. -/
#eval show CoreM Unit from do
  ...
  for n in declared.toList do
    unless frozen.contains n do
      throwError "guard VIOLATION: Axioms.lean declares {n}, \
        which is not in the frozen 32-axiom whitelist (commit 20e2d14). \
  ...
  IO.println s!"guard 1: Axioms.lean declares exactly the 32 frozen axioms ✓"
  ...
  for n in axioms do
    if n == ``sorryAx then
      ...
      throwError "guard VIOLATION: kernel_sound uses axiom {n}, which is \
        ...
        32 frozen axioms of Axioms.lean). Adding an axiom requires human \
        sign-off."
  if sorries then

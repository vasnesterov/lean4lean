...
hypothesis about `env` and no `Faithful`.  `VIndRestore.NestedBarrier` (just below §2)
instantiates `P` at the concrete `_nested` test.

**Relocated 2026-09-03.**  This file's old §1 (`VExpr.NoConstIn` and its lemmas) and old §3
(`IsNestedName` and its `Lean.Name` lemmas) now live in
`Lean4Lean/Theory/Inductive/NestedNames.lean`, verbatim.  They are spec-level vocabulary, and
`Theory/Inductive/NestedBuild.lean` needs them to state `VNestedOcc.OccursN` — which it cannot
do from here, since this file imports it.  The section numbers below are left as they were so
that the cross-references in other files' docstrings keep pointing at the same text.

...
- **Keep the checker close to the official C++ kernel — that outranks shrinking the trusted base.** Replacing an upstream `opaque`/`partial`/`@[extern]` function with a pure Lean one is **not** a goal in itself and is **not** progress on its own: closeness to the standard kernel matters more. Sometimes such a change is genuinely unavoidable; when it is, keep it as small as possible, prefer it over abandoning a needed proof, and record the resulting behavioural difference from the C++ kernel — including complexity blowups on inputs it handles differently — in `divergences.md`. Performance is not a priority, so long as the Kernel Arena still passes in reasonable time.
- Corollary, since this was got wrong on 2026-09-04: when a proof stalls on an upstream opaque — `Lean.Expr.eqv` being `@[extern]`, `PersistentHashMap.containsAux`/`findAux` being `partial` — **restating the obligation around it is the preferred route**, and replacing the function is the fallback, not the first move. "Unprovable by design because it is an extern" is a real constraint on the proof, not an invitation to rewrite the kernel.
